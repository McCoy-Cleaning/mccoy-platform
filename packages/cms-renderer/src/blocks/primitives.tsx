...
function ButtonPopupDialog({
  ...
  const [BlockView, setBlockView] = React.useState<
    null | typeof import("./RegisteredBlockView").RegisteredBlockView
  >(null);

  React.useEffect(() => {
    if (!open) return;
    let cancelled = false;
    void import("./RegisteredBlockView").then((mod) => {
      if (!cancelled) setBlockView(() => mod.RegisteredBlockView);
    });
    return () => {
      cancelled = true;
    };
