...
describe("buildStaffAuthAppLink", () => {
  it("builds a direct /invite URL with token_hash and type", () => {
    const url = buildStaffAuthAppLink({
      redirectTo: "https://admin.example.com/invite",
      hashedToken: "abc123",
      type: "invite",
    });
    expect(url).toBe("https://admin.example.com/invite?token_hash=abc123&type=invite");
  });

  it("preserves existing query params on redirectTo", () => {
    const url = buildStaffAuthAppLink({
      redirectTo: "https://admin.example.com/invite?lang=nl",
      hashedToken: "tok",
      type: "recovery",
    });
...
describe("resolveStaffAuthEmailLink", () => {
  it("prefers app link over Supabase action_link", () => {
    ...
    expect(link).toBe("https://admin.example.com/invite?token_hash=hashed&type=invite");
  ...
  it("falls back to action_link with redirect_to when hashed_token is missing", () => {
    ...
    expect(link).toBe(
      "https://xxx.supabase.co/auth/v1/verify?token=legacy&type=recovery&redirect_to=https%3A%2F%2Fadmin.example.com%2Finvite",
    );
  });
...
describe("withInviteRedirectTo", () => {
  it("sets redirect_to on action links", () => {
    const out = withInviteRedirectTo(
      ...
      "https://admin.example.com/invite",
    );
    expect(out).toContain("redirect_to=https%3A%2F%2Fadmin.example.com%2Finvite");
  });
});
