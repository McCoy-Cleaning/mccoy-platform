...
const BUNDLED_HERO_WEBP_SRCSET = `${heroWebp640} 640w, ${heroWebp960} 960w, ${heroWebp1280} 1280w`;
const HOME_GALLERY_FALLBACKS = [
  "/images/cms/work-regular.jpg",
  "/images/cms/work-horeca.jpg",
  "/images/cms/work-oplevering.jpg",
  "/images/cms/work-floor.jpg",
  "/images/cms/about-vision-alt.jpg",
...
export function Hero() {
  ...
  return (
    <section id="home" className="relative isolate flex min-h-[100svh] items-center overflow-hidden pt-24">
      ...
      {/* Accent blobs — avoid infinite JS rotation on the LCP path */}
      <motion.div
        aria-hidden
          ...
          <h1 className="font-display mt-6 text-6xl leading-[0.98] tracking-tight text-white sm:text-7xl lg:text-[5.75rem] xl:text-[6.5rem]">
            ...
            <span className="relative inline-block bg-gradient-to-br from-primary via-primary to-white/90 bg-clip-text text-transparent">
              ...
              <motion.span
                ...
                animate={{ scaleX: 1 }}
                transition={{ delay: reducedMotion ? 0 : 0.9, duration: reducedMotion ? 0 : 0.9, ease: [0.22, 1, 0.36, 1] }}
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/15 shadow-[0_30px_80px_-20px_rgba(63,182,242,0.4)]">
              {content.image.decorative ? (
                <DeliveryImage
                  ...
                  alt=""
                  role="presentation"
                  webpSrcSet={imageSrc === hero ? BUNDLED_HERO_WEBP_SRCSET : undefined}
                  {...heroImgProps}
                />
              ) : (
                <DeliveryImage
...
export function WorkGallery() {
  ...
  const galleryItems = copy.items.map((item, i) => ({
    id: item.id,
    src: isCmsPlaceholderSrc(item.image.src) ? fallbackImages[i] || HOME_GALLERY_FALLBACKS[0] : item.image.src,
