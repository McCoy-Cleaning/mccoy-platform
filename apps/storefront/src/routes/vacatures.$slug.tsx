...
function findVacancyBySlug(vacancies: VacancyItem[], slug: string): VacancyItem | null {
  ...
  return (
    vacancies.find((v) => resolveVacancyPublicSlug(v).toLowerCase() === normalized) ?? null
  );
}
    ...
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <main className="mx-auto max-w-3xl px-4 pb-20 pt-32 sm:px-6 lg:px-8">
        ...
        <article className="mt-6 rounded-3xl border border-white/10 bg-card/50 p-6 sm:p-10">
          {vacancy.featured ? (
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Uitgelicht</p>
          ) : null}
          ...
          {vacancy.shortDescription ? (
            <p className="mt-6 text-base leading-relaxed text-white/80">{vacancy.shortDescription}</p>
          ) : null}
          ...
          {vacancy.requirements?.length ? (
            <section className="mt-6">
              <h2 className="text-sm font-semibold uppercase tracking-wider text-white/45">Eisen</h2>
              <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-white/75">
