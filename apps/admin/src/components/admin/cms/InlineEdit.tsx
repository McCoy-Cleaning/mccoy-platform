...
import { CMS_MAX_SOURCE_IMAGE_BYTES, prepareCmsImageUpload } from "@mccoy/cms-schema";
...
async function fileToDataUrl(file: File): Promise<string> {
  const prepared = await prepareCmsImageUpload(file, { profile: "photo" });
  if (!prepared.ok) {
  ...
  const handle = async (f?: File | null) => {
    ...
    try {
      ...
      notifyToast({
        ...
        description: err instanceof Error ? err.message : "Probeer een ander bestand.",
      });
    } finally {
      setBusy(false);
    }
    ...
    >
      {src ? (
        ...
        <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-xs text-white/50">
          ...
          <span className="text-[10px] text-white/30">max {maxMb}MB · wordt gecomprimeerd</span>
        </div>
      )}
      <button
        type="button"
      ...
      >
        <span className="inline-flex items-center gap-2 rounded-lg border border-white/20 bg-black/50 px-3 py-1.5 text-xs font-medium text-white">
          ...
          {busy ? "Comprimeren…" : src ? "Vervangen" : "Uploaden"}
        </span>
      </button>
      {src && (
        <button
